...
public class MainTest2 {
    ...
    public void SendingLetter(){
        try{
            ...
            Thread.sleep(20000);//ожидает 20 секунд до закрытия окна сообщения об отправке письма
        }
        catch (Exception e){
            ps.println("Ошибка: " + e + "\n");
            b = false;
    ...
    public void LetterWithoutAddressSubject(){
        try{
            ...
            //закрывает форму письма
            driver.findElement(By.xpath("//button[@class='ControlButton-Button']")).click();
            Assert.assertTrue(b, a + "\n");
            ps.println("3. Совпадение\n");
        }
        catch (Exception e){
